...
public class NGTObject {
	...
	public NBTTagCompound writeToNBT() {
		...
		for (int i = 0; i<this.blockList.size(); ++i) {
			BlockSet set = this.blockList.get(i);
			if (!idMap.containsKey(set)) {
				idMap.put(set, idCount);
				++idCount;
			}

			int id = idMap.get(set);
			blockIds[i] = id;
	...
	private static NBTTagCompound decompress(NBTTagCompound data) {
		...
		try {
			return CompressedStreamTools.func_152457_a(compressedData, new NBTSizeTracker(4194304L));
		} catch (IOException e) {
			e.printStackTrace();
		}
