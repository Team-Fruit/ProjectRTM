...
import net.teamfruit.projectrtm.rtm.item.ItemAmmunition.BulletType;
...
public class EntityBullet extends EntityArrow {
	...
	//EntityArtillery
	public EntityBullet(final World world, final BulletType type, final double x, final double y, final double z, final double mX, final double mY, final double mZ) {
		this(world);
		this.yOffset = 0.0F;
		setBulletType(type);
		setPosition(x, y, z);
		...
		this.motionY = mY;
		this.motionZ = mZ;
		setThrowableHeading(this.motionX, this.motionY, this.motionZ, 10.0F, 0.0F);
	}

	//ItemGun
	public EntityBullet(final World world, final EntityLivingBase shooter, final float speed, final BulletType type) {
		this(world);
		this.yOffset = 0.0F;
		setBulletType(type);
		...
		final float yawRad = NGTMath.toRadians(this.rotationYaw);
		final float pitchRad = NGTMath.toRadians(this.rotationPitch);
	...
	//ItemGun(NPC)
	public EntityBullet(final World world, final EntityLivingBase shooter, final EntityLivingBase target, final float speed, final BulletType type) {
		this(world);
		this.shootingEntity = shooter;
		...
		final double distance = MathHelper.sqrt_double(d0*d0+d2*d2);

		if (distance>=1.0E-7D) {
			final float f2 = (float) NGTMath.toDegrees(Math.atan2(d2, d0))-90.0F;
			final float f3 = (float) -NGTMath.toDegrees(Math.atan2(d1, distance));
			final double d4 = d0/distance;
			final double d5 = d2/distance;
			setLocationAndAngles(shooter.posX+d4, this.posY, shooter.posZ+d5, f2, f3);
			this.yOffset = 0.0F;
			setThrowableHeading(d0, d1, d2, speed, 1.0F);
		}
	}
	...
	private void setBulletSize() {
		if (getBulletType()==BulletType.cannon_40cm)
			setSize(0.5F, 0.5F);
		else
			setSize(0.05F, 0.05F);
	}
	...
	public void setThrowableHeading(double par1, double par3, double par5, final float par7, final float par8) {
		final double f2 = Math.sqrt(par1*par1+par3*par3+par5*par5);
		final double d0 = par7/f2;
		par1 *= d0;
		par3 *= d0;
		par5 *= d0;
		this.motionX = par1;
	...
	public void onUpdate() {
		...
		final Block block = this.worldObj.getBlock(this.tileX, this.tileY, this.tileZ);
		if (block.getMaterial()!=Material.air) {
			block.setBlockBoundsBasedOnState(this.worldObj, this.tileX, this.tileY, this.tileZ);
			final AxisAlignedBB aabb = block.getCollisionBoundingBoxFromPool(this.worldObj, this.tileX, this.tileY, this.tileZ);
			if (aabb!=null&&aabb.isVecInside(Vec3.createVectorHelper(this.posX, this.posY, this.posZ)))
				this.inGround = true;
		}

		if (this.inGround) {
			final int j = this.worldObj.getBlockMetadata(this.tileX, this.tileY, this.tileZ);
			...
			++this.ticksInAir;
			final Vec3 vecPos = Vec3.createVectorHelper(this.posX, this.posY, this.posZ);
			...
			if (list.size()>0) {
				final Vec3 vecPos2 = Vec3.createVectorHelper(this.posX, this.posY, this.posZ);
				double d0 = 0.0D;
				for (int i = 0; i<list.size(); ++i) {
					...
					if (entity.canBeCollidedWith()&&flag) {
						...
						final AxisAlignedBB aabb = entity.boundingBox.expand(0.5D, 0.5D, 0.5D);
						final MovingObjectPosition mop1 = aabb.calculateIntercept(vecPos2, vec3);
						if (mop1!=null) {
							final double d1 = vecPos2.distanceTo(mop1.hitVec);
							if (d1<d0||d0==0.0D) {
								hitEntity = entity;
								d0 = d1;
							}
			...
			final BulletType type = getBulletType();
	...
	public void writeEntityToNBT(final NBTTagCompound nbt) {
		...
		nbt.setByte("canBreak", (byte) (getCanBreakBlock() ? 1 : 0));
		nbt.setByte("bulletType", getBulletType().id);
	}
	...
	public void readEntityFromNBT(final NBTTagCompound nbt) {
		...
		setCanBreakBlock(nbt.getByte("canBreak")==1);
		setBulletType(BulletType.getBulletType(nbt.getByte("bulletType")));
	}
	...
	public void setBulletType(final BulletType par1) {
	...
	public void setCanBreakBlock(final boolean par1) {
	...
	protected void onHitEntity(final Entity entity) {
		//entity.setFire(5);
		final float damage = getBulletType().damage;
	...
	/**着弾時のアクション*/
	protected void onLanding(final int x, final int y, final int z) {
		final boolean doMobGriefing = this.worldObj.getGameRules().getGameRuleBooleanValue("mobGriefing");
		final BulletType type = getBulletType();
		if (type==BulletType.cannon_40cm) {
			if (!this.worldObj.isRemote) {
					...
					;
				{
					...
					this.worldObj.newExplosion(this, x+0.5D, y+0.5D, z+0.5D, 12.0F, false, doMobGriefing);
				}
				setDead();
			}
			...
			if (!this.worldObj.isRemote) {
				...
				if (block!=null&&hardness>0.0F&&hardness<500.0F)
					;
				{
					this.worldObj.setBlock(x, y, z, RTMBlock.effect);
					this.worldObj.playSoundEffect(x+0.5D, y+0.5D, z+0.5D, "random.explode", 4.0F, (1.0F+(this.worldObj.rand.nextFloat()-this.worldObj.rand.nextFloat())*0.2F)*0.7F);
				}
				setDead();
			}
			...
			} else if (isBreakableBlock(getBulletType(), this.landingBlock, x, y, z)) {
				final int meta = this.worldObj.getBlockMetadata(x, y, z);
	...
	protected boolean isBreakableBlock(final BulletType bullet, final Block block, final int x, final int y, final int z) {
		final boolean doMobGriefing = this.worldObj.getGameRules().getGameRuleBooleanValue("mobGriefing");
		...
		if (bullet==BulletType.handgun_9mm) {
			...
			mat==Material.wood||mat==Material.leaves||mat==Material.plants
					||mat==Material.vine||mat==Material.circuits||mat==Material.carpet
					||mat==Material.glass||mat==Material.redstoneLight||mat==Material.coral
