...
public class EntityBogie extends Entity implements Lockable {
	...
	public void updateBogie() {
		...
		if (this.worldObj.isRemote) {
			if (this.carPosRotationInc>0) {
				double d0 = 1.0D/(double) this.carPosRotationInc;
				double x = this.posX+(this.carX-this.posX)*d0;
				double y = this.posY+(this.carY-this.posY)*d0;
				double z = this.posZ+(this.carZ-this.posZ)*d0;
				float yaw = this.rotationYaw+MathHelper.wrapAngleTo180_float(this.carYaw-this.rotationYaw)*(float) d0;
				float pitch = this.rotationPitch+(this.carPitch-this.rotationPitch)*(float) d0;
	 ...
	 * @return 衝突して止まるならtrue
	 */
	private boolean collideWithEntities(List par1) {
		EntityTrainBase train = this.getTrain();

		Iterator iterator = par1.iterator();
